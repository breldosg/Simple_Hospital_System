...
export class ViewActivePharmacyListView {
    constructor() {
        this.PatientData = [];
    ...
    populateTable(PatientData) {
        ...
        PatientData.VisitList.forEach((patient, index) => {
            ...
            row.innerHTML = `
                    ...
                    <p class="orders">${patient.pharmacy_orders}</p>
            ...
            row.addEventListener('click', () => {
                frontRouter.navigate('/pharmacy/activevisits/' + patient.id);
            })
    ...
    async fetchData() {
        try {
            const response = await fetch('/api/pharmacy/search_visit_with_pharmacy_order', {
                method: 'POST',
                headers: {

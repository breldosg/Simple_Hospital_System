...
export class ViewActiveLaboratoryListView {
    constructor() {
        this.PatientData = [];
    ...
    populateTable(PatientData) {
        ...
        PatientData.VisitList.forEach((patient, index) => {
            ...
            row.addEventListener('click', () => {
                frontRouter.navigate('/laboratory/activevisits/' + patient.id);
            })
    ...
    async fetchData() {
        try {
            const response = await fetch('/api/laboratory/search_visit_with_laboratory_order', {
                method: 'POST',
                headers: {

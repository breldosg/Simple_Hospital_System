...
export class ViewActiveBillsListView {
    constructor() {
        this.PatientData = [];
    ...
    populateTable(PatientData) {
        ...
        PatientData.VisitList.forEach((patient, index) => {
            ...
            row.addEventListener('click', () => {
                frontRouter.navigate('/billing/activebills/' + patient.id);
            })
    ...
    async fetchData() {
        try {
            const response = await fetch('/api/billing/search_active_visit_with_bill_order', {
                method: 'POST',
                headers: {
    ...
    ViewReturn() {
        ...
        <div class="main_section active_radiology_list">
            <div class="in_table_top d_flex flex__u_b">
                <h4>Active Visit List With Bill</h4>

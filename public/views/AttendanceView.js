...
export class AttendanceView {
    ...
    async PreRender() {
        ...
        this.main_container = document.querySelector('.attendance_cont');
    ...
    async fetchAndRenderData() {
        ...
        if (!this.page_shift) {
            const roleData = await this.fetchRole(); // Fetch category data only once
            this.roleData = roleData || [];


            const roles = (role_raw) => {
                var rolesElem = `
                <br-option type="checkbox" value=" ">Select Role</br-option>
                `;
            ...
            this.role_elements = roles(this.roleData);
    ...
    async fetchData() {
        try {
            const response = await fetch('/api/users/search_attendance', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    async fetchRole() { 
        try {
            ...
            return result.success ? result.data.roles : null;
        } catch (error) {
            console.error('Error:', error);
            notify('top_left', error.message, 'error');
            return null;
    ...
    searchMedicineView() {
        ...
        <br-form callback="search_medicine">
            <div class="medicine_content">
                <br-input label="User Name" name="query" type="text" value="${this.searchTerm == null ? '' : this.searchTerm}" placeholder="Enter user name" styles="
                ...
                <br-select search name="role" fontSize="13px" label="Role" value="${this.role_value}" placeholder="Select Role" styles="
                                    border-radius: var(--input_main_border_r);
                                    width: 400px;
                                    padding: 10px;
                                    height: 41px;

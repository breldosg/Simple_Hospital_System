...
export class CreateServicePopUp {
    constructor() {
        window.CreateServicePopUp = this.CreateService.bind(this);
    ...
    async PreRender(params = '') {
        ...
        this.main_container = document.querySelector('.create_service_popup');
    ...
    ViewReturn(loader = '') {
        const title = this.isUpdate ? "Update Service" : "Create Service";
        const submitText = this.isUpdate ? "Update" : "Submit";
        return `
        <div class="container create_service_popup">
            <br-form class="slides anima" callback="CreateServicePopUp">
                <div class="slide">
                    <div>
                        <p class="heading">${title}</p>
                    </div>
                    <div class="input_group">
                        <br-input required name="name" label="Name" type="text" value="${this.serviceData?.name || ''}" styles="${this.input_styles()}" labelStyles="font-size: 12px;"></br-input>
                        <br-select required name="status" fontSize="13px" label="Status" placeholder="Select Status" styles="${this.input_styles()}" labelStyles="font-size: 12px;" value="${this.serviceData?.status || 'active'}">
    ...
    async CreateService(data) {
        ...
        const endpoint = this.isUpdate ? '/api/billing/update_service' : '/api/billing/create_service';
        ...
        try {
            ...
            if (result.success) {
                ...
                if (dashboardController.servicesManagementSettingView) {
                    dashboardController.servicesManagementSettingView.render();
                }
            } else {
    ...
    async fetchData() {
        // No longer fetch categories, just return null
        return null;
    }
